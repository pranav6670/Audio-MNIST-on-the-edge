...
def plot_fft(fft):
    ...
    fig.canvas.set_window_title('Fourier Transforms')
    i = 0
    for x in range(2):
        for y in range(4):
...
def plot_fbank(fbank):
    ...
    fig.canvas.set_window_title('Filter Bank Coefficients')
    i = 0
    for x in range(2):
        for y in range(4):
...
for f in df.index:
    rate, signal = wavfile.read('recordings/'+f)
...
fig, ax = plt.subplots()
fig.canvas.set_window_title('Class Distributions')
...
for c in classes:
    ...
    signal, rate = librosa.load('recordings/'+wav_file, sr=44100)
